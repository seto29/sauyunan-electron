...
export const getAllOnlyLatest = async (dateFrom, dateUntil) => {
    const response = await axios.get('/paySells/GetAllOnlyLatest.php?dateFrom='+dateFrom+"&dateUntil="+dateUntil)
    return response.data
}

export const getAllOnlyLatestGiro = async (dateFrom, dateUntil) => {
    const response = await axios.get('/paySells/GetAllOnlyLatestGiro.php?dateFrom='+dateFrom+"&dateUntil="+dateUntil)
    return response.data
}
export const getAllGiroNot = async () => {
    const response = await axios.get('/paySells/GetGiroNotCashOut.php')
    return response.data
}
...
export const fInsert = async (kode_penjualan,nama_pelanggan, kode_pelanggan, harga, jumlah_bayar, sisa, kode_transaksi, komisi, tanggal_bayar,  jumlah_retur, no_giro1, bank1, nilai_giro1, tanggal_cair1, no_giro2, bank2, nilai_giro2, tanggal_cair2, no_giro3, bank3, nilai_giro3, tanggal_cair3, jumlah_potongan, kota, telepon, alamat_pelanggan) => {
    var bodyFormData = new FormData();
